...
/*#include "/home2/antonio/redneu_v7/babel_v6/rna.hpp"
#include "/home2/antonio/redneu_v7/babel_v6/ecs_v6.hpp"
#include "/home2/antonio/redneu_v7/babel_v6/basics.hpp"
#include "/home2/antonio/redneu_v7/babel_v6/random.hpp"*/
...
int main(int argc, const char* argv[]){
    ...
    /**Valor de seleccion de la primera generacion**/
    A = ecs.adec_LE(objetivo, seqs[0], rn, lig, delta);
    adec_prom.push_back(A);
    adec_max.push_back(A);
    ...
    T_obje  = make_tree(st1); //aqui esta la ss de tao
    free(st1);
    ...
    prob_mfe_sd[0].push_back(rna.probab_reperplastic_mfe(pob_inicial, objetivo, delta)); //return de funcion es float
    ...
    for(int t = 1; t < generaciones; ++t){
        ...
        for(int j = 0; j < n_mol; ++j){
            if(plegar[j] == 1){
                ...
                adec[j] = ecs.adec_LE(objetivo, seqs[j], rn, lig, delta);
                free(st2);
                free_tree(T2); 
            }
        }
        ...
        /* Calcular robustez promedio y maxima */
        if(t == (generaciones - 1)){
            for(int i = 0; i < n_mol; ++i){
        ...
        c = v[v.size() - 1]; //valor de la ultima casilla
        for(int i = 0; i < n_mol; ++i){
            u                   = c*alea.uno();
    ...
    path    = "/home2/antonio/redneu_v7/res/R1/";
